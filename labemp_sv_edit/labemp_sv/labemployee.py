...
- (Sinh viên tự viết) Giảm lương cho nhân viên (decreaseSalary), biết rằng số lương giảm phải lớn hơn 0
...
- Opt-19: Lưu danh sách nhân viên xuống file dbemp_output.db, biết rằng mỗi nhân viên là một dòng
...
while(True):
 ...
 # - Opt-4: Hiển thị thông tin của một nhân viên khi biết mã nhân viên
            ...
            find_code = input("Input code: ")
            flag = 0
            for find_tt in dsNhanVien:
                if (find_tt.getCode() == find_code):
                    input("profile of Employee to find: ")
                    find_tt.display()
                    flag = 1
                    break
            if (flag == 0):
...
# - Opt-5: Chỉnh sửa thông tin một nhân viên
            ...
            find_SetCode = input("code of employee: ")
            flag = 0
            for list in dsNhanVien:
                if (list.getCode() == find_SetCode):
                    editName = input("Name to edit: ")
                    editAge = input("Age to edit: ")
                    editSalary = input("Salary to edit: ")
                    list.setName(editName)
                    list.setAge(int(editAge))
                    list.setSalary(float(editSalary))
                    list.display()
                    flag = 1
                    break
            if (flag == 0):
...
# - Opt-6: Xóa một nhân viên ra khỏi danh sách
            ...
            delEpl = input("code of employee want to delete: ")
            flag = 0
...
# - Opt-7: Tăng lương cho một nhân viên
            ...
            upSalEpl = input("employee code who wants a raise : ")
            salUp = input ("the amount of wages that want to increase: ")
            flag = 0

            for i in dsNhanVien:
                if (i.getCode() == upSalEpl):
                    i.increaseSalary(float(salUp))
                    print("done!")
                    flag = 1
                    break
            if (flag == 0 ):
...
# - Opt-8: Giảm lương cho một nhân viên
            ...
            downSalEpl = input("employee code who wants a diminish : ")
            salDown = input ("the amount of wages that want to diminish: ")
            flag = 0

            for i in dsNhanVien:
                if (i.getCode() == downSalEpl):
                    i.diminishSalary(float(salDown))
                    print("done!")
                    flag = 1
                    break
            if (flag == 0 ):
...
# - Opt-9: Tính số lượng nhân viên (countEmp) và xuất ra màn hình
            ...
            countEpl = 0
            # for  i in len(dsNhanVien):
            #     countEpl += 1
            print("count of employee: "+ str(len(dsNhanVien)))
